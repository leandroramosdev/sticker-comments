...
(function (factory, global) {
    ...
    if (event) {
      ...
      do {
        ...
        if (
          ...
          eventStorage[self.id][_event].length
        ) {
  ...
  function getTemplate(template, unicode, shortname) {
    ...
    template = template
      ...
      .replace(
        "{img}",
        imagePath +
          (emojioneSupportMode < 2 ? fname.toUpperCase() : fname) +
          "." +
          imageType
      )
      .replace("{uni}", unicode);
  ...
  function getDefaultOptions() {
    ...
    var supportMode = !emojione
      ? getSupportMode(getEmojioneVersion())
      : getSupportMode(detectVersion(emojione));
    if (supportMode > 4) {
      defaultOptions.filters = {
        tones: {
          title: "Diversity",
          emoji:
            "open_hands raised_hands palms_up_together clap pray thumbsup thumbsdown punch fist left_facing_fist right_facing_fist " +
        ...
        flags: {
          ...
          title: "Bandeiras",
          emoji:
            "ac af al dz ad ao ai ag ar am aw au at az bs bh bd bb by be bz bj bm bt bo ba bw br bn bg bf bi " +
  ...
  function unicodeTo(str, template) {
    return str.replace(uniRegexp, function (unicodeChar) {
      var map =
        emojione[emojioneSupportMode === 0 ? "jsecapeMap" : "jsEscapeMap"];
      if (typeof unicodeChar !== "undefined" && unicodeChar in map) {
        return getTemplate(
          template,
          map[unicodeChar],
          emojione.toShort(unicodeChar)
        );
      }
      return unicodeChar;
    });
  ...
  function lazyLoading() {
    ...
    if (
      !self.sprite &&
      self.lasyEmoji[0] &&
      self.lasyEmoji.eq(0).is(".lazy-emoji")
    ) {
  ...
  function div(prefix) {
    var parent = $(
      "<div/>",
      isObject(prefix) ? prefix : { class: selector(prefix, true) }
    );
    ...
    if (!self.recent || self.recent !== emojis || show) {
      if (emojis.length) {
        ...
        if (!skinnable) {
          ...
          height = self.recentCategory.is(":visible")
            ? self.recentCategory.height()
            : 0;
        }

        var items = shortnameTo(emojis, self.emojiBtnTemplate, true)
          .split("|")
          .join("");
        self.recentCategory.children(".emojibtn").remove();
        $(items).insertAfter(
          self.recentCategory.children(".emojionearea-category-title")
        );
  ...
  function init(self, source, options) {
    ...
    self.emojiBtnTemplate =
      '<i class="emojibtn" role="button" data-name="{name}" title="{friendlyName}">' +
      self.emojiTemplateAlt +
      "</i>";
    self.recentEmojis = options.recentEmojis && supportsLocalStorage();

    var pickerPosition = options.pickerPosition;
    self.floatingPicker =
      pickerPosition === "top" || pickerPosition === "bottom";
    self.source = source;
    ...
    var sourceValFunc =
        source.is("TEXTAREA") || source.is("INPUT") ? "val" : "text",
      editor,
      button,
      picker,
      filters,
      ...
      app = div(
        {
          class:
            css_class +
            (self.standalone ? " " + css_class + "-standalone " : " ") +
            (source.attr("class") || ""),
          role: "application",
        },
        (editor = self.editor =
        ...
        (button = self.button =
          div("button", div("button-open"), div("button-close")).attr(
            "title",
            options.buttonTitle
          )),
        (picker = self.picker =
          div(
            ...
            div(
              ...
              (scrollArea = div(
                "scroll-area",
                options.tones && !options.search
                  ? div("tones-panel", tones)
                  : null,
            ...
            .addClass(
              selector("picker-position-" + options.pickerPosition, true)
            )
            .addClass(
              selector("filters-position-" + options.filtersPosition, true)
            )
            .addClass(
              selector("search-position-" + options.searchPosition, true)
            )
            .addClass("hidden"))
      );
    ...
    var mainBlock = div("category-block")
      .attr({ "data-tone": 0 })
    ...
    $.each(options.filters, function (filter, params) {
      ...
      if (
        filter !== "tones" &&
        script_values.categories_removed.includes(filter) == false
      ) {
        $("<i/>", {
          class:
            selector("filter", true) + " " + selector("filter-" + filter, true),
          "data-filter": filter,
          title: params.title,
        })
      ...
      do {
        ...
        if (script_values.categories_removed.includes(filter) == false) {
          if (filter === "stickers") {
            ...
            if (arr.length > 0) {
              arr.map((item) => {
                items =
                  items +
                  '<i class="emojibtn" role="button" data-name=":smile:" title="Smile"><img class="emojioneemoji lazy-emoji" data-src="' +
                  `${script_values.site_url + item}` +
                  '"/></i>';
              });
            }
          } else {
          ...
          category.html(items);
          $('<div class="emojionearea-category-title"/>')
            .text(params.title)
            .prependTo(category);
        }
      } while (--skin > 0);
    ...
    if (self.standalone && !self.getText().length) {
      var placeholder =
        $(source).data("emoji-placeholder") || options.emojiPlaceholder;
    ...
    scrollArea.on("scroll", function () {
      if (!noListenScroll) {
        ...
        if (scrollArea.is(":not(.skinnable)")) {
          ...
          var filter = filtersBtns.filter(
            '[data-filter="' + item.attr("name") + '"]'
          );
    ...
    self
      .on("@filter.click", function (filter) {
        ...
        var headerOffset = categories
            .filter('[name="' + filter.data("filter") + '"]')
            .offset().top,
      ...
      .on("@!paste", function (editor, event) {
        ...
        window.setTimeout(function () {
          ...
          var text = textFromHtml(
            clipboard.html().replace(/\r\n|\n|\r/g, "<br>"),
            self
          );
          clipboard.remove();
          pasteText(text);
      ...
      .on("@emojibtn.click", function (emojibtn) {
        ...
        let sticker = emojibtn[0].innerHTML;
        if (sticker.includes("/emojione/assets/") == false) {
          sticker = sticker.replace("emojioneemoji", "emojioneemoji-sticker");
        }
    ...
    if (options.search) {
      self
        ...
        .on("@search.keypress", function (hide) {
          ...
          var activeTone = options.tones
            ? tones.find("i.active").data("skin")
            : 0;
          ...
          if (term && term.length) {
            ...
            categoryBlocks.each(function () {
              var matchEmojis = function (category, activeTone) {
                var $matched = category.find(
                  '.emojibtn[data-name*="' + term + '"]'
                );
                if ($matched.length === 0) {
                  ...
                  filterBtns
                    .filter('[data-filter="' + category.attr("name") + '"]')
                    .hide();
                } else {
                  var $notMatched = category.find(
                    '.emojibtn:not([data-name*="' + term + '"])'
                  );
                  $notMatched.hide();
    ...
    if (options.autocomplete) {
      var autocomplete = function () {
        ...
        var map = $.map(emojione.emojioneList, function (_, emoji) {
          return !options.autocompleteTones
            ? /_tone[12345]/.test(emoji)
              ? null
              : emoji
            : emoji;
        });
          ...
          [
            {
              ...
              template: function (value) {
                return (
                  shortnameTo(value, self.emojiTemplate) +
                  " " +
                  value.replace(/:/g, "")
                );
              },
        ...
        if (options.textcomplete.placement) {
          ...
          if (
            $(editor.data("textComplete").option.appendTo).css("position") ==
            "static"
          ) {
            $(editor.data("textComplete").option.appendTo).css(
              "position",
              "relative"
            );
          }
    ...
    if (!cdn.isLoading) {
      if (!emojione || getSupportMode(detectVersion(emojione)) < 2) {
        ...
        if (getSupportMode(emojioneVersion) > 5) {
          emojioneJsCdnUrlBase =
            cdn.defaultBase3 + "npm/emojione@" + emojioneVersion;
        } else if (getSupportMode(emojioneVersion) > 4) {
          emojioneJsCdnUrlBase =
            cdn.defaultBase3 + "emojione/" + emojioneVersion;
        } else {
        ...
        $.ajax({
          ...
          success: function () {
            ...
            if (emojioneSupportMode > 4) {
              cdn.base =
                cdn.defaultBase3 + "emojione/assets/" + emojioneVersion;
              sprite =
                cdn.base +
            ...
            if (options.sprite) {
              if (document.createStyleSheet) {
                ...
                $("<link/>", { rel: "stylesheet", href: sprite }).appendTo(
                  "head"
                );
              }
            }
    ...
    emojioneReady(function () {
      ...
      if (options.useInternalCDN) {
        ...
        emojione.imagePathSVGSprites =
          cdn.base + "/sprites/emojione.sprites.svg";
        ...
        $.each($.isArray(ev[0]) ? ev[0] : [ev[0]], function (i, el) {
          $(el).on(ev[1], function () {
            var args = slice.call(arguments),
              target = $.isFunction(ev[2])
                ? ev[2].apply(self, [event].concat(args))
                : ev[2];
            if (target) {
  ...
  EmojioneArea.prototype.on = function (events, handler) {
    if (events && $.isFunction(handler)) {
      ...
      $.each(events.toLowerCase().split(" "), function (i, event) {
        ...
        (
          eventStorage[self.id][event] || (eventStorage[self.id][event] = [])
        ).push(handler);
      });
    }
    return this;
  ...
  $.fn.emojioneArea = function (options) {
    return this.each(function () {
      ...
      $.data(
        this,
        "emojioneArea",
        (this.emojioneArea = new EmojioneArea($(this), options))
      );
  ...
  $.fn.emojioneAreaText = function (options) {
    ...
    var self = this,
      pseudoSelf = {
        shortnames:
          options && typeof options.shortnames !== "undefined"
            ? options.shortnames
            : true,
        emojiTemplate:
          '<img alt="{alt}" class="emojione' +
          (options && options.sprite && emojioneSupportMode < 3
            ? '-{uni}" src="' + blankImg
            : 'emoji" src="{img}') +
          '"/>',
      };
